...
## v8 — 2026-05-31 — Deploy-safe modal logo + staging deploy to minghwee2
- **Fixed the typeform modal logo 404.** `sg.js` builds the onboarding modal via `innerHTML` with a literal `images/logo.png`. The website-pages deploy rewrites HTML asset refs into the per-page deployment subdir but NOT paths baked into JS strings, so the modal logo 404'd at `/images/logo.png`. Now resolved at runtime: prefer an already-deployed `<img src*="logo.png">` in the DOM (correct for whatever layout the deploy produced), else derive from `sg.js`'s own script URL (`assets/` → `../images/`); falls back to the original relative path. Deploy-layout-agnostic — no behaviour change in the source/local context.
- **Deployed to staging** `minghwee2.reputifly.cloud` (Archimedes v1.67.1): page_id 6, isolated raw-HTML, set as the static front page. Verified — all 14 assets HTTP 200 byte-identical to source, 0 console errors, modal logo loads (naturalWidth 4500, no 404), hero + 3 helper-card images render 1:1.
- Files changed: assets/sg.js, CHANGELOG.md
- Agent: Claude Opus 4.8

## v7 — 2026-05-28 — Typeform layout: centre + fill more space
- **Fixed vertical centering bug.** The engine showed each question by setting `stageEl.style.display = 'block'`, which overrode `.tf-stage`'s `display:flex` and killed the `align-items/justify-content:center` — so questions were top-stuck with empty space below (worst on mobile). Now sets `display:flex`; `.tf-stage` is a centered flex column. Questions sit centred (balanced space above/below).
- **Fills more space.** Card max-width 540→620px (desktop). Option buttons enlarged (padding 14/16→17/20px, font 15→16px, gap 8→10px) for a fuller, more tappable feel. On mobile: tighter side padding (18→16px) for wider content, card max-width 100%, options 18px padding / 16px font / 15px radius, larger markers, taller textarea, bigger underline inputs.
- Verified on mobile (375px): question card centred, options full-width. sg.js re-validated.
- Files: assets/sg.js (1-line display fix), assets/sg.css (tf-stage/tf-card/tf-opt + mobile block), CHANGELOG.md.
