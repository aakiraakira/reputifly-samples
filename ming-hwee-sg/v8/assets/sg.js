    ...
    root.innerHTML =
      '<div class="tf-shell">' +
        '<div class="tf-header">' +
          '<div class="tf-brand">' +
            '<img src="' + (function () {
              // Deploy-safe logo URL. The website-pages deploy rewrites asset
              // refs inside HTML but NOT paths baked into JS strings, so a
              // literal "images/logo.png" 404s under the deployment subdir.
              // Resolve at runtime: prefer an already-deployed logo <img> in
              // the DOM (its .src is correct for whatever layout the deploy
              // produced), else derive from sg.js's own URL (assets/ -> ../images/).
              var el = document.querySelector('img[src*="logo.png"]');
              if (el && el.src) return el.src;
              var ss = document.getElementsByTagName('script');
              for (var i = ss.length - 1; i >= 0; i--) {
                if (ss[i].src && /sg\.js(\?|$)/.test(ss[i].src)) {
                  try { return new URL('../images/logo.png', ss[i].src).href; } catch (e) {}
                }
              }
